...
public class Applications {
    ...
    private static class AppComparator implements Comparator {
        ...
        public int compare(Object o1, Object o2) {
            CyanfoxAppInfo a1 = (CyanfoxAppInfo) o1;
            CyanfoxAppInfo a2 = (CyanfoxAppInfo) o2;
    ...
    private static final String APPEND_CMD = "echo \"%s=%s\" >> /system/etc/cyanfox/properties.conf";
    private static final String REPLACE_CMD = "busybox sed -i \"/%s/ c %<s=%s\" /system/etc/cyanfox/properties.conf";
    private static final String PROP_EXISTS_CMD = "grep -q %s /system/etc/cyanfox/properties.conf";
    ...
    private static List<CyanfoxAppInfo> appList = new ArrayList();
    ...
    public static void addApplication(Context mContext, CyanfoxAppInfo app, int dpi) {
...
//                Utils.restartUI();
    ...
    public static CyanfoxAppInfo[] getApplicationList(Context mContext, int dpi) {
        ...
        try {
            ...
            properties.load(new FileInputStream("/system/etc/cyanfox/properties.conf"));
        } catch (Throwable t) {
        ...
        List<CyanfoxAppInfo> items = new ArrayList();
        ...
        while (it.hasNext()) {
            ...
            if (packageName.endsWith(".dpi") && sdpi.equals(currentdpi)) {
                CyanfoxAppInfo bAppInfo = findAppInfo(mContext, packageName);
        ...
        return items.toArray(new CyanfoxAppInfo[items.size()]);
    }

    public static CyanfoxAppInfo[] getApplicationList(Context mContext) {
        ...
        CyanfoxAppInfo[] items = new CyanfoxAppInfo[mPackageList == null ? 0 : mPackageList.size()];

        appList.clear();

        for (int i = 0; mPackageList != null && i < mPackageList.size(); i++) {
            ApplicationInfo app = mPackageList.get(i);
            items[i] = new CyanfoxAppInfo();
    ...
    public static void backup(Context mContext) {
        Utils.execute(new String[] {
                ...
                "mkdir files",
                "chmod 777 files",
                "cp /system/etc/cyanfox/properties.conf " + BACKUP,
                "chmod 644 " + BACKUP
        }, 0);
    ...
    public static void restore(Context mContext) {
        Utils.execute(new String[] {
                ...
                "cp " + BACKUP + " /system/etc/cyanfox/properties.conf",
                "chmod 644 /system/etc/cyanfox/properties.conf",
    ...
    private static CyanfoxAppInfo findAppInfo(Context mContext, String packageName) {
        ...
        for (int i = 0; i < appList.size(); i++) {
            CyanfoxAppInfo app = appList.get(i);
    ...
    private static String read() throws Exception {
        ...
        FileInputStream fstream = new FileInputStream("/system/etc/cyanfox/properties.conf");
